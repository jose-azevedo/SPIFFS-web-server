 ...
 * -Armazenamento em arquivos separados para cada sistema
 * -Correção da posição da chamada da função de limpar variáveis de média (dados não zeram mais)
 * 
 * FALTA CRIAR VARIAVEIS ACUMULADORAS PRA CORRENTE E TENSÃO RMS DC E ATUALIZAR A FUNÇÃO DE SALVAR DADOS NO CARTÃO
*/
...
if (SD.begin())Serial.print("Cartão SD inicializado");
else  {
  Serial.print("Falha ao inicializar o cartão SD");
  return;
}
...
// Geração do nome do arquivo
  date0 = String(gen) + String(d) + "-" + String(m) + "-" + String(y) + ".csv" ;
  date0.toCharArray(fileDate, 14);
...
void sendData(){
  ...
  fileData = String(rtc.getTimeStr()) + " ; " + String(acc_avgI_DC[i]/acc) + " ; " + String(acc_rmsI_DC[i]/acc) + " ; " + String(acc_avgV_DC[i]/acc) + " ; " + String(acc_rmsV_DC[i]/acc) + " ; " + String(accP_DC[i]/acc) + " ; " + String(acc_rmsI_AC[i]/acc) + " ; " + String(acc_rmsV_AC[i]/acc) + " ; " + String(accS[i]/acc) + " ; " + String(accFP[i]/acc);
...
t = rtc.getTime(); // Variável recebedora da informação de tempo
if (t.min > wait) wait = (t.min/interval + 1)*interval; // Os dados só são salvos em minutos múltiplos de 5
if (wait > t.min + interval) wait = 0; // Condição para reiniciar o ciclo quando min0 superar 55
Serial.print(t.min);
Serial.print("\t");
Serial.println(wait);
if (t.min == wait && x == 1){ // Salvar os dados após o tempo de espera e somente quando após os cálculos de AC2
  Serial.print("Entrou no laço");
  for (i=0; i<=1; i++){
    fileCreation();  
    sendData();
  }
